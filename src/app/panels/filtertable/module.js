...
function (angular, app, _, kbn, moment) {
  ...
  var module = angular.module('kibana.panels.filtertable', []);
  app.useModule(module);

  module.controller('filtertable', function($rootScope, $scope, $modal, $q, $compile, $timeout,
    ...
    var rolFilter = function(data, filter){
        var flag = true;
        if(!_.isEmpty(filter)) {
          for(var key in filter){
            var value = data.kibana._source[key];
            if(_.isUndefined(value)){
              value = "undefined";
            }
            if (filter[key] && filter[key][value] == false) {
              flag = false;
              return;
            }
          }
    ...
    $scope.countBy = function(data, field) {
      var isInit = false;
      if(!$scope.tableFilters[field]) {
        isInit = true;
        $scope.tableFilters[field] = {};
      }

      var result = countBy(data, function(d){
        return rolFilter(d,$scope.tableFilters) ? {key:d.kibana._source[field],flag: true} : {key:d.kibana._source[field],flag: false};
      });

      result = _.sortBy(_.pairs(result), function (value) {
        if(isInit) {
          $scope.tableFilters[field][value[0]] = true;
        } else {
          if(_.isUndefined($scope.tableFilters[field][value[0]]))  $scope.tableFilters[field][value[0]] = true;
        }
        return -1 * value[1]
      });
      var sum = $scope.filterResultLen =  result.reduce(function(sum, r) {
        return sum + r[1];
      }, 0 );
      return result;
    };
    ...
    $scope.allTrue = function(list){
      _.forEach(list,function(v,k){
        list[k] = true;
      })
    };

    $scope.allFalse = function(list){
      _.forEach(list,function(v,k){
        list[k] = false;
      })
    };
    ...
    $scope.isHasFalse = function(list, field) {
      var r = false;
      _.forEach(list,function(v,k){
        if(list[k] == false){
          r = true;
...
//        callback = lodash.createCallback(callback, thisArg, 3);
        ...
        if (typeof length == 'number') {
          while (++index < length) {
            ...
            r = callback(value, index, collection);
            setter(result,  r.flag, r.key, collection);
          }
        } else {
          forOwn(collection, function(value, key, collection) {
            r = callback(value, key, collection);
            setter(result, r.flag, r.key, collection);
          });
        }
        return result;
    ...
    var countBy = createAggregator(function(result, flag, key) {
      if(!flag) {
        !hasOwnProperty.call(result, key) && (result[key] = 0);
      } else {
        hasOwnProperty.call(result, key) ? result[key]++ : result[key] = 1;
      }
    });

    $scope.setShowHeaderFilter = function(value){
      $scope.showHeaderFilter = value;
    }
  });
  ...
  module.filter('tableFilter', function () {
    return function (input, filter) {
      if(!_.isUndefined(input)) {
        var result = [];
        _.forEach(input, function (data) {
          var flag = true;
          if(!_.isEmpty(filter)) {
            for(var key in filter){
              var value = data.kibana._source[key];
...
//          console.log(element.offset(), element.scrollTop());
          if (!element.hasClass('panel-scroll') && scrollTop > attrs.panelScroll) {
            element.addClass('panel-scroll');
            scope.$apply(function(){
              scope.setShowHeaderFilter(true);
            })
          } else if (element.hasClass('panel-scroll') && scrollTop < attrs.panelScroll) {
            element.removeClass('panel-scroll');
            scope.$apply(function(){
              scope.setShowHeaderFilter(false);
            })
          }
        }
        element[0].onmousewheel = onScroll
      }
    }
  });
